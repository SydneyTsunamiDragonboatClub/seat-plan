...
import CreateRaceCategory from "./modals/CreateRaceCategory";
...
const Tabs: React.FC<PaddlerTabType> = ({ categories }) => {
    ...
    return (
        <div>
                    ...
                    {strCategories.map(category => (
                        <li className="mr-2" key={category}>
                            <a href="#" aria-current="page"
                                ...
                                onClick={(e) => tabClicked(category)}>{category}</a>
                        </li>
                    ))}
                </ul>
            ...
            {showCategoryModal && (
                <CreateRaceCategory categories={strCategories} addCategory={handleAddcategory} updateCategoryModal={handleCategoryModal} />
            )}
        </div >
    )
};
